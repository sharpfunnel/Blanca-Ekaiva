 ...
 * Meta Pixel loader.
 *
 * A client component rather than a bare <Script> in the layout, because it has
 * two jobs the server cannot do: keep the pixel off /admin (staff traffic is
 * not audience data), and re-fire PageView on client-side navigation, which the
 * one-shot inline snippet never sees.
 *
 * Renders nothing at all when NEXT_PUBLIC_META_PIXEL_ID is unset, so an
 * environment without the id ships no snippet.
 */
